...
Файлы в `./data` используются для кешей и управляемых учетных записей. В Docker volume `./data:/app/data` должен быть доступен на запись пользователю контейнера `node` (`uid 1000` в официальном Node-образе). При деплое создайте директорию заранее и проверьте права, иначе запись кеша или пользователей будет падать с `EACCES`.
