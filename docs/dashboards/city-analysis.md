...
## Вкладка `Рейтинг актуальных городов`

Вкладка показывает города с заказами за выбранный период и помогает выбрать целевые локации для маркетинга и ОМПП. Запрос к ClickHouse выполняется один раз на период и возвращает строки уровня `город + бренд`; фильтрация по бренду и сортировка по колонкам выполняются в браузере без повторных запросов к базе.

Метрики:

- `Заказ` - сумма `mg_orders.amount` по актуальным заказам города.
- `Точки с заказами` - количество рабочих мест с заказами.
- `Бренды` - количество брендов в текущем срезе.
- `SLA` - отношение закрытого спроса к заказу. Закрывающими считаются смены из `mg_jobs` в статусах `booked`, `going`, `inprogress`, `checkingin`, `checkingout`, `completed`, `delayed`, `waiting`, а также успешные `confirmed` по общему helper с исключением прогулов.

Для быстрого UI бренд не добавляется в SQL-фильтр рейтинга. При выборе бренда таблица пересобирается из уже загруженных строк `город + бренд`.
