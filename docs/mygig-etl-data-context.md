...
Правило по умолчанию: для дашбордов сначала использовать `mg_*`; к `mygig_*` обращаться только если в `mg_*` нет аналога нужного поля.
...
Важно для дашборда продаж по проектам: факт выполненных смен за актуальные периоды считать из `mg_jobs`, а не из `mg_job_history.start`. Проверка 2026-06-01 по маю 2026 показала, что `mg_job_history.start` часто пустой/непригодный для календарного фильтра и дает сильный недосчет `confirmed` смен. `mg_job_history` в этом дашборде использовать точечно для признаков событий, например самоброни (`status = 'booked'` и `initiator = 'worker'`), связывая историю с уже выбранными сменами через `mg_job_history.job = mg_jobs._id`.

Проверка сырого слоя `mygig_*` для майских данных 2026: `mygig_jobs` по полю `start` заканчивается 2024 годом, `mygig_job_history` - 2025-06-01, поэтому эти таблицы не являются корректным источником факта выполненных смен за май 2026. Для текущей аналитики использовать `mg_jobs`.
...
Для факта успешно выполненной смены в большинстве отчетов использовать не только `status = 'confirmed'`, но и проверку, что смена не является прогулом. `confirmed`-смена с длительностью `0:00` и нулевым начислением/выплатой считается прогулом и не участвует в SLA, выручке, выполненных сменах и метриках успешного выполнения. Практическое условие для обычной смены: `status = 'confirmed'` и есть хотя бы один положительный признак факта - `hours`, `payment`, `salary_per_job`, `salary_per_hour * hours` или интервал `start_fact < finish_fact` с `dateDiff('minute', start_fact, finish_fact) > 0`. Для смен типа сделка признак брать из связанного заказа `mg_orders.pieceworks`: если он не пустой, нулевая фактическая оплата клиенту считается прогулом, даже если остальные признаки заполнены. Для операционной воронки использовать также `booked`, `going`, `inprogress`, `checkingout`, `completed`, `cancelled`, `failed`.

Расшифровка статусов смен из продуктового справочника:

| Статус | Значение | В деталях смены | В координации |
| --- | --- | --- | --- |
| `doccheck` | Смена забронирована, но документы еще на проверке. Если будет гигер с уже подтвержденными документами, он сможет забронировать смену первым. | На проверке документов | Проверка документов |
| `booked` | Смена забронирована, исполнитель видит ее в расписании. | Забронирована | Назначен |
| `going` | Исполнитель подтвердил, что придет. | В пути | Подтвердил |
| `delayed` | Исполнитель опаздывает. | Опаздывает | Запросил опоздание |
| `waiting` | Исполнитель опаздывает, заказчик подтвердил, что подождет. | Ожидается | Опаздывает |
| `toolate` | Исполнитель опаздывает, заказчик отменил смену и не стал ждать. | Отмена из-за опоздания | Отмена из-за опоздания |
| `checkingin` | Исполнителю нужно зачекиниться в начале. | Нет чекина в начале | Проблема с чекином в начале |
| `inprogress` | Смена идет. | Выполняется | Работает |
| `checkingout` | Исполнителю нужно зачекиниться в конце. | Нет чекина в конце | Проблема с чекином в конце |
| `completed` | Смена завершена, деньги не начислены. | Завершена | Завершена |
| `confirmed` | Смена завершена и подтверждена, деньги начислены. | Подтверждена | Подтверждена |
| `failed` | Смена не выполнена: исполнитель не зачекинился или отменил. | Не выполнена | Не выполнена |
