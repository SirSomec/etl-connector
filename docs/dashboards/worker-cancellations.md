...
По нажатию на ФИО открывается модальное окно с актуальными чёрными списками исполнителя. Списки клиентов берутся из `mg_clients.blacklist`, списки рабочих мест — из `mg_workplaces.blacklist`; элементы массивов связаны с исполнителем через `worker_id`.

В окне также выводится последнее событие `mg_operators_history` с `model = 'ban_list'`: оператор и время в МСК. Это справочная информация, а не атрибут конкретного включения: журнал не хранит идентификатор клиентского/точечного списка и не различает добавление и удаление.

Если актуальные массивы не содержат исполнителя, но событие `ban_list` совпадает по времени с операторской отменой его смены (расхождение не более пяти минут), окно показывает клиента, рабочее место и юрлицо этой смены как контекст события. Такая строка явно помечается как восстановленный контекст и не считается подтверждением текущего состояния списка.

## Предзагруженная витрина

Витрина `worker-cancellations` хранится в общем SQLite preload store. Ежедневно в `04:00 Europe/Moscow` она выбирает исполнителей с хотя бы одной актуальной сменой за последние 60 дней и заново записывает для них факты смен, признаки отмен, данные поиска, фильтров и детализации. Будущие даты в плановое окно не входят.

Список исполнителей, варианты фильтров и детализация смен читаются из витрины, когда она полностью покрывает запрошенный период. Для периода вне покрытия, при отсутствии SQLite-файла или ошибке чтения используется прежний ClickHouse fallback.

## Доменные правила

- В метрики попадают только смены, связанные с актуальным заказом через `mg_jobs.source = mg_orders._id`.
- Актуальный заказ: не удален, не скрыт, клиент не входит в список тестовых клиентов, тип контракта не `processing`.
- Контрагент определяется через рабочее место заказа: `mg_orders.workplace -> mg_workplaces._id -> mg_contractors._id`.
- Бренд и рабочее место в детализации берутся из заказа, а не из полей смены, чтобы отмененные/устаревшие смены не подтягивали неверный контекст.
