...
test('loadHeatmapDashboardSection loads worker concentration only when layer is enabled', async () => {
  ...
  assert.match(concentrationCall.query, /concentration_candidates/);
  assert.match(concentrationCall.query, /quantileExact\(0\.5\)\(density_per_km2\) AS p50_density_per_km2/);
  assert.match(concentrationCall.query, /quantileExact\(0\.95\)\(density_per_km2\)/);
  assert.match(concentrationCall.query, /density_per_km2 > p50_density_per_km2/);
  assert.match(concentrationCall.query, /\(density_per_km2 - p50_density_per_km2\) \/ \(p95_density_per_km2 - p50_density_per_km2\)/);
  assert.match(concentrationCall.query, /AS intensity/);
  assert.doesNotMatch(concentrationCall.query, /min_density_per_km2/);
