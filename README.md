...
В дашбордах по умолчанию используются таблицы `mg_*`. Для `Продажи по проектам` проектом считается бренд клиента из `mg_clients.title`. В `Анализе точек` стабильность считается как доля дней с плановым заказом по `mg_orders.amount`. В `Анализе городов` город выбирается как город точки заказа, а база исполнителей считается по последней локации пользователя в радиусе 15 км от точек с выбранным спросом за период и фильтры.
