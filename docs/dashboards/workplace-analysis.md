...
- Все метрики заказов и смен используют актуальные заказы: не удаленные, не скрытые, без тестовых клиентов и без `processing`.
- Контрагент определяется через рабочее место заказа: `mg_orders.workplace -> mg_workplaces._id -> mg_contractors._id`.
- Смены для SLA, прогноза и отмен связаны с заказом через `mg_jobs.source = mg_orders._id`; raw `mg_jobs` без актуального заказа в метрики не попадает.
...
- Негеографические агрегатные `CROSS JOIN` заменены на обычные join к агрегатным CTE; гео-расчеты радиусов оставляют декартово сопоставление только там, где оно ограничено расстоянием.

## Операционный риск

В подвкладке `Требуют внимания` риск точки отображается как `Высокий`, `Средний` или `Низкий`. Это вычисляемая в приложении оценка, она не добавляет отдельный SQL-запрос и строится из уже загруженных полей ближайших 7 дней: незакрытого заказа, покрытия, максимального дневного пика и активной базы в радиусе 15 км.

Причины риска показываются только когда соответствующий сигнал есть в данных. Например, интерфейс может показать свободный заказ, низкое покрытие, недостаточную активную базу на свободную смену или дневной пик незакрытых смен. Если данных для причины нет, интерфейс не выводит ее как факт.

В карточке точки календарь дней получает визуальный уровень риска из дневных значений: высокий риск при SLA ниже 50% или отвале за 24 часа, средний риск при SLA ниже 80%, низкий риск в остальных случаях. Это помогает операционному менеджеру быстро перейти от общей очереди внимания к конкретному проблемному дню.
