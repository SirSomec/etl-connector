...
test('loadBrandAnalysisDashboardSection maps regions with the same demand and fulfillment metrics as region cities', async () => {
  const { calls, client } = createDashboardClient({
    ...
    'brand analysis region order trend': [
      ...
      { region: 'ЦФО', period: '2026-04-03', ordered_shifts: 12 }
    ]
  });
  ...
  assert.deepEqual(dashboard.regionRows, [{
    ...
    orderTrend: [
      ...
      { period: '2026-04-03', orderedShifts: 12 },
      { period: '2026-04-04', orderedShifts: 0 },
      { period: '2026-04-05', orderedShifts: 0 }
    ]
  }]);
  assert.deepEqual(calls.map((call) => call.operation), [
    'brand analysis region orders',
  ...
  assert.match(calls[2].query, /toDate\(o\.start\) AS period/);
