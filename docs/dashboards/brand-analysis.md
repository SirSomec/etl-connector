...
- `regions` - регионы присутствия бренда: заказ, свободный заказ, SLA, покрытие, выполнение и число точек; дневная полоса рядом с регионом показывает динамику заказа за выбранный диапазон независимо от группировки основного графика. Дни без заказа выделены синим, положительные значения нормализуются отдельно внутри каждого региона по шкале красный–желтый–зеленый.
