...
- `regions` - регионы присутствия бренда: заказ, свободный заказ, SLA, покрытие, выполнение и число точек; градиентная полоса рядом с регионом показывает динамику заказа по выбранной группировке периода.
