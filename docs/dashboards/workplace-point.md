...
- В каждый запуск попадают все точки, у которых есть хотя бы одна актуальная заявка в `mg_orders` за последние 14 дней на момент запуска preload; дополнительно сохраняются точки из зарегистрированных `preload_dashboard_requests`.
